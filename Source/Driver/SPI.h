...
#define	SSP_MLSB_MSB						0<<0		//MSB transmit first
#define	SSP_MLSB_LSB						1<<0		//LSB transmit first
#define	mskSSP_MLSB_MSB 				(SSP_MLSB_MSB<<0)
#define	mskSSP_MLSB_LSB 				(SSP_MLSB_LSB<<0)

																		//[1:1]Clock polarity selection bit
#define	SSP_CPOL_SCK_IDLE_LOW		0<<1		//SCK idles at Low level
#define	SSP_CPOL_SCK_IDLE_HIGH	1<<1		//SCK idles at High level
...
#define	SSP_CPHA_FALLING_EDGE		0<<2		//Data changes at clock falling edge
#define	SSP_CPHA_RISING_EDGE		1<<2		//Data changes at clock rising edge
...
#define	SSP_DIV  								24//48		//MCLK/n, n = 2, 4, 6, 8, ...,512
...
#define	__SPI0_FIFO_RESET		(SN_SPI0->CTRL0_b.FRESET = SSP_FRESET_RESET_FIFO);  
#define	__SPI0_OUTPUTMODE_SEL0 	 SN_GPIO0->MODE |= (0x01<<5);	//set P05 is ouput mode
#define	__SPI0_SET_SEL0			 SN_GPIO0->BSET |= (0x01<<5);
#define	__SPI0_CLR_SEL0			 SN_GPIO0->BCLR |= (0x01<<5);

#define	__SPI0_SET_SEL1			 SN_GPIO0->BSET = 0x02;
#define	__SPI0_CLR_SEL1			 SN_GPIO0->BCLR = 0x02;

#define	__SPI0_SET_SEL2			 SN_GPIO3->BSET = 0x10;
#define	__SPI0_CLR_SEL2			 SN_GPIO3->BCLR = 0x10;
//SSP Data Fetch speed (High: SCK>6MHz)
#define	__SSP0_DATA_FETCH_HIGH_SPEED	(SN_SPI0->DF = SSP_DF_EN)		//*(volatile unsigned long *)(0x4001C020) = 1
#define	__SSP1_DATA_FETCH_HIGH_SPEED	(SN_SPI0->DF = SSP_DF_EN)		//*(volatile unsigned long *)(0x40058020) = 1



#define SPI_ASSERT                {__SPI0_SET_SEL0; \
                                   __NOP();              \
                                   __NOP();              \
                                   __NOP();              \
																	 __NOP();              \
...
void SPI0_Init(void);
void SSP0_InterruptEnable(void);

extern void SPI_Burst_Read(uint8_t bRegAddr);
extern uint8_t SPI_R_BYTE(uint8_t bRegAddr);
extern void SPI_W_2BYTE( uint8_t bRegAddr, uint8_t bData);
extern void SPI0_Disable(void);
extern void SPI0_Enable(void);
extern void SPI_W_NBYTE(uint8_t bRegAddr, uint8_t* pdata, uint16_t blength, uint8_t bCS);
#endif	/*__SN32F240_SSP_H*/

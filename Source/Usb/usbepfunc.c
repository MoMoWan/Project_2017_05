	...
	if ((*pUsbReg & (mskEPn_ENDP_EN|mskEPn_ENDP_STATE)) == (mskEPn_ENDP_EN|mskEPn_ENDP_STATE_NAK))
	{
		if (wUSB_EndpHalt[USB_EP1] == USB_EPn_NON_HALT)
		{
	      ...
	      //b_aryKB_Mouse[6] = sensorIntegrity;
				fnUSBMAIN_WriteFIFO( wEPn_Address , (( b_aryKB_Mouse[3]<<24)|(b_aryKB_Mouse[2]<<16) |b_aryKB_Mouse[1]<<8) | b_aryKB_Mouse[0]);				
				fnUSBMAIN_WriteFIFO( wEPn_Address + 4 , ((0x00<<24)|b_aryKB_Mouse[6]<<16|(b_aryKB_Mouse[5]<<8) | b_aryKB_Mouse[4]  ));
...
//				{}
				...
				USB_EPnAck(USB_EP1,7);				//** ACK hwByteCnt byte	

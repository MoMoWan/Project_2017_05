...
void USB_EP0InEvent(void)
{
	...
	{	
		...
		if( (sHID_Data.ReportFeature.wHID_Setreportfeature[0] == RETURN_KERNEL_0) &&(sHID_Data.ReportFeature.wHID_Setreportfeature[1] == RETURN_KERNEL_1))
		{	
				//** check ISP password
				//** Clear USB FIFO
				for(i=0; i<64 ; i++)	
...
{	
	...
	if(sHID_Data.wHID_SetReportFeature == mskSET_REPORTFEATURE_FLAG)	//** Check Set_report_feature == 1
	{
		...
		for(i=0; i<16 ; i++)
		{
			...
			sHID_Data.ReportFeature.wHID_Setreportfeature[i] = wUSBINT_ReadDataBuf;
		}
		...
		{
			//SetFeatureReport 
			bAPP_HIDStatus |= mskUSB_SetReportFeatureArrival;
			
			//Data Read In sHID_Data.ReportFeature.b_aryAPP_SetReportFeature[]
		}
	...
	else if (sHID_Data.wHID_Status & mskSET_REPORT_FLAG)	//** Check Set_report_flag == 1
	{
		...
		fnUSBINT_ReadFIFO(0x00);
		sHID_Data.wHID_SetRptByte[0] = ((wUSBINT_ReadDataBuf>>8)&0xff); //** save data

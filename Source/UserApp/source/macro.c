...
U8  Macro_Ep1_In[MACRO_EP1_SIZE];                        // Macro end point 1 in 
U8  Macro_Ep2_In[MACRO_EP2_SIZE];                        // Macro end point 2 in 
U8  Macro_Ep3_In[MACRO_EP3_SIZE];                        // Macro end point 3 in 
...
--*/
{
	...
	if(offsetCheck==offset){                                     // Check offset	
		...
		if((addr)&&(dataSize)&&(flashCRCVal==0xffff)){             // Data ,Name in not 0 , crc is default value
			...
			if(addr&EXTER_ADDR_FLAG){                                // External flash
				...
				if(size+cnt<DATA_BUFFER_SIZE){                         // Buffer is not full , keep filled buffer
					...
					ret=flashPageWrite(addr+pageIndex*PAGE_SIZE,protocolBuffer); // Write flash
				...
				if(offset+size>=dataSize){                             // Reach the the end bytes
					if(cnt+CRC_SIZE<=DATA_BUFFER_SIZE){                  // Add the crc bytes (2) still not full
						...
						ret=flashPageWrite(addr+pageIndex*PAGE_SIZE,protocolBuffer);  // Write Inter flash
						pageIndex=0;                                       // Clear
						cnt=0;	                                           // Clear
						wait=0;		                                         // CLear
						crc=0xffff;		                                     // Reset
            offsetCheck=0;						                         // Clear
						...
						memcpy(&protocolBuffer[cnt],&crc,DATA_BUFFER_SIZE-cnt);  // Copy 
 						flashPageWrite(addr+pageIndex*PAGE_SIZE,protocolBuffer); // Write the flash 
    ...
    if(macroTable.macro[i].id.id==id){                            // ID match		
      if(macroTable.macro[i].dataBlockAddr!=0){
        if(macroTable.macro[i].dataBlockAddr&EXTER_BLOCK_FLAG){   // Exter flash .check data
          ...
          crc_cal=crc16((U8*)(macroTable.macro[i].dataBlockAddr<<8),macroTable.macro[i].dataSize);  // Calculate the crc 
          memcpy((U8*)&crc,(U8*)((macroTable.macro[i].dataBlockAddr<<8)+macroTable.macro[i].dataSize),2); // Read the crc bytes 
        ...
        if(macroTable.macro[i].nameBlockAddr!=0){
          if(macroTable.macro[i].nameBlockAddr&EXTER_BLOCK_FLAG){   // Exter flash .check data
            ...
            crc_cal=crc16((U8*)(macroTable.macro[i].nameBlockAddr<<8),macroTable.macro[i].nameSize);
            memcpy((U8*)&crc,(U8*)((macroTable.macro[i].nameBlockAddr<<8)+macroTable.macro[i].nameSize),2);
            //crc=*((U16*)((macroTable.macro[i].dataBlockAddr<<8)+macroTable.macro[i].dataSize));
            if(crc_cal==crc){
              ret=TRUE;
            }else{
...
--*/
{
	...
	if(macroActive[idx].popCount!=macroActive[idx].readcount){                          // popIndex not equal pushIndex, not finish current cycle
        ...
        //]        
				break;
			case POWER_MAKE:
				Macro_Ep2_In[0]=0x03;         // Media report id :0x02
        ...
        //]        
			...
			case MEDIA_MAKE:                    // Media make even
				Macro_Ep2_In[0]=0x02;         // Media report id :0x02

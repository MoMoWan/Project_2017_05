...
U8 randomCompFlag = 0;
U8 ProfileNo = 0;
U8 previousEffect[NUMBER_OF_LEDS]={0,0,0,0};                                      // Prevous effect 
  ...
  U8 i;
	switch (led[0].pendingeffect) {
      ...
      if (led[0].Chroma_effect.colorNO == 0) {                      // if it a ramdom color selected
        ...
        comp = ledParameters(1, ramPr.profile[ramPr.ProNO].savedLight[0].id);// get the address of the first lighting index
  ...
  if (led[0].tmr >= led[0].scratch)  {                    // If time to update Pulse or Stage, then...
    led[0].tmr = 0;                                       // Restar time counter.
      ...
      if ((led[0].state & RISING) != 0) {                 // If the pulse is rising, then...
        if (led[0].pwm < ram.breathParam.pulseNeckLine) {
          pulseStep = ram.breathParam.pulseRise1Step;
        } else {
          pulseStep = ram.breathParam.pulseRise2Step;
        }
        if ((0xFFFF - led[0].pwm) > pulseStep) {            // If we haven't reached the top, then...
          led[0].pwm += pulseStep;                          // Add in a full step value
          led[0].scratch = ram.breathParam.pulseRiseTime;            // Next processing after 'Step Period'
          ...
          led[0].scratch = ram.breathParam.pulseOn;                  // Next processing after 'On Period'
        }
      } else {                                              // If the pulse is falling, then...
        if (led[0].pwm < ram.breathParam.pulseNeckLine) {
          pulseStep = ram.breathParam.pulseFall1Step;
        } else {
          pulseStep = ram.breathParam.pulseFall2Step;
        }
        if (led[0].pwm > pulseStep) {                       // If we haven't reached the bottom, then...
          led[0].pwm -= pulseStep;                          // Substract off a full step value
          led[0].scratch = ram.breathParam.pulseFallTime;            // Next processing after 'Step Period'
          ...
          led[0].scratch = ram.breathParam.pulseOff;                 // Next processing after 'Off Period'	
  ...
  if (mixV != 0) {                                 // if current was "ON" then 
       ...
       if (led[0].effect == CHROMA_EFX_BREATHE) {
         if (lighting[led[0].index].extendeffect == MATRIXDISSOLVE) {
           ...
           led[0].pwm = lighting[led[0].index].pwm;       // when breathe set fade out, the lighting pwm copy to ramPr
         }
       }
	...
	if(led[0].extendTimer >= 10){
      ...
      led[0].extendeffect = NO_EFFECT;
			if (led[0].effect ==  CHROMA_EFX_BREATHE ) {        // breathing effect will start from on to breath out after fade in. 
			   led[0].pwm = 0xFFFF;                              // Set top value
         led[0].state &= ~RISING;                          // Prepare for falling pulse after 'On Period'
         led[0].scratch = ram.breathParam.pulseOn + 1490;                  // Next processing after 'On Period
			 }
		}
		updateMatrixHardware(led[0].id ,ON);
  }
...
{
  U8 i;
  LED_REFRESH_TIME =  ramPr.profile[ramPr.ProNO].Runing_light_LED_REFRSH_Time; 
  for (i = 0; i < NUMBER_OF_LEDS; i++) {
	  ramPr.profile[ramPr.ProNO].savedLight[i].index = i;                   // make sure the index is correct
    Load_LED_Effect(&ramPr.profile[ramPr.ProNO].savedLight[i]); 	        // load LED effect
...
--*/
{
  ...
  s = (U8 *)&sys.cont.maxLight[led[0].index];             // Source = 'saved' calibration values
...
{
  U8 i = 0;
  while ((ramPr.profile[ramPr.ProNO].savedLight[i].id != led_id) && (i < NUMBER_OF_LEDS)) {    
  ...
  if ((profile_id == 0) || (i == NUMBER_OF_LEDS)) {       // If Class #0 or invalid LED ID, then...
    ...
    return (&ramPr.profile[profile_id-1].savedLight[i]);  // Return 'saved' LED parameter address
  }
}

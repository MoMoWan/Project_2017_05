...
bool  newProfile(U8 id)
{
	...
	if((0<id)&&(id<=profileNO)){        // Id range from 1 -5
	  if((ramPr.aliveProfile&(1<<(id)))==0){ // profile id is not exist
		  ramPr.aliveProfile|=(1<<(id));
		  ret=TRUE;
	  }
   }
	if(id==1){                                // Special deal ,because the profile 1 will not be deleted , so if profile name is set , need to release space here 
		if(ramPr.profile[id-1].nameAdd!=0){                   // Has the profile name , need to release the space to recycleable 
			//readFlash((U32)(ramPr.profile[id-1].nameAdd),(U8*)&nameSize,2);	   // Get the profile size
			nameSize=ramPr.profile[id-1].nameSize+CRC_SIZE;               // add the crc bytes and the size bytes
			if(ramPr.profile[id-1].nameAdd&EXTER_ADDR_FLAG){     // Extern flash
	...
	if(ret==TRUE||id==1){             // Need to back to default setting
		ramPr.profile[id-1]=dft_generic2.profile[id-1];
...
bool deleteProfile(U8 id)
{
	bool ret=FALSE;
	U16 nameSize;
	if(((ramPr.aliveProfile&(1<<(id)))==(1<<id))&&(id!=1)){ // profile id is not exist , profile 1 can not be delete
		ramPr.aliveProfile&=~(1<<(id));
		if(ramPr.profile[id-1].nameAdd!=0){                   // Has the profile name , need to release the space to recycleable 
			//readFlash((U32)(ramPr.profile[id-1].nameAdd),(U8*)&nameSize,2);	   // Get the profile size
			nameSize=ramPr.profile[id-1].nameSize+CRC_SIZE;               // add the crc bytes and the size bytes
	...
	switch (type){
	    ...
	    for(i=ramPr.PClass+1;;i++){
          ...
          for(i=0;i<NUMBER_OF_LEDS;i++){
            ramPr.profile[ramPr.ProNO].savedLight[i].index=i;
          }
          for(i=0;i<NUMBER_OF_LEDS;i++){
            updateProfileLed(&ramPr.profile[ramPr.ProNO].savedLight[i],ramPr.PClass);
            copyLighting2Active(&ramPr.profile[ramPr.ProNO].savedLight[i]);       // copy ram to light	           
          }
		...
		case PROFILE_DOWN:
          ...
          loadProfileLed();   // Load the profile indicate led
			...
			break;
		case PROFILE_NUM:
			if((ramPr.aliveProfile>>num)&0x01){    // is alive
				ramPr.PClass=num;
				ramPr.ProNO=ramPr.PClass-1;
        for(i=0;i<NUMBER_OF_LEDS;i++){
          ramPr.profile[ramPr.ProNO].savedLight[i].index=i;
...
U32 getProfileNameAddr(U8 id)
{
	...
	for(i=0;i<profileNO;i++){
		if(ramPr.profile[i].profile_ID==id/*&&(ramPr.profile[i].nameStage==true)*/){
			ret=ramPr.profile[i].nameAdd;
			break;
		}
	}
	return ret;
...
bool getProfileName(U8 id,U8* totalSize,U16 offset,U8* pData,U8 size)
{
    ...
    if(ramPr.profile[id-1].nameStage==true){        // If the profile name has been setting
      addr=ramPr.profile[id-1].nameAdd;             // Get profile addr
      if(addr!=0){
       // readFlash(addr,(U8*)&nameSize,2);	   // Get the profile size
        nameSize=ramPr.profile[id-1].nameSize;
...
//	static U32 offsetCheck=0;
	...
	U8 ret=0;
	if(!(ramPr.aliveProfile&(1<<(id)))){ // profile id is not exist ,return fail 
		return ret;
	}
		...
		if(blockNum!=0){       // Get the address is right		
		  if(ramPr.profile[id-1].nameAdd!=0){                   // Has the profile name , need to release the space to recycleable 
			 ...
			 // nameSize+=(CRC_SIZE+2);               // add the crc bytes and the size bytes
			  if(ramPr.profile[id-1].nameAdd&EXTER_ADDR_FLAG){     // Extern flash
				  macroTable.recycleSizes+=((ramPr.profile[id-1].nameSize+2)/SECTOR_SIZE+1)*SECTOR_SIZE;
			  }else{
				  macroTable.recycleSizes+=((ramPr.profile[id-1].nameSize+2)/PAGE_SIZE+1)*PAGE_SIZE;
			  }	
			  ramPr.profile[id-1].nameAdd=0;
				ramPr.profile[id-1].nameSize=0;
			  requestVariableUpdate(MACRO_CHANGE,0);	
		  }
			if(blockNum==EXTER_BLOCK_FLAG){  // 
				ramPr.profile[id-1].nameAdd=(blockNum*SECTOR_SIZE);
			}else{
				ramPr.profile[id-1].nameAdd=(blockNum*PAGE_SIZE);
			}
			ramPr.profile[id-1].nameSize=totalSize;
      ramPr.profile[id-1].nameCrc=0;                        // Clear the crc bytes 
      ramPr.profile[id-1].nameStage=false;                  // Disable the profile name 
			...
			if(offset+size>=nameSize&&(wait!=WAIT_ONE)){    // Reach the end of the data ,need to wirte
				...
				if(!sstIsBusy()){            // Flash is not busy
					if(cnt+CRC_SIZE<=DATA_BUFFER_SIZE){                   // Add crc size and not full yet
						...
						ret=RETURN_SUCCESS;
						wait=0;
            ramPr.profile[id-1].nameCrc=crc;
            ramPr.profile[id-1].nameStage=true;
            requestVariableUpdate(SW_CHANGEDPr,id-1);	
						crc=0xffff;
//								offsetCheck=0;
...
//					 offsetCheck=0;
		...
		if(size+cnt<DATA_BUFFER_SIZE){ // Buffer is not full , keep filled buffer
			...
			memcpy(&protocolBuffer[cnt],pData,DATA_BUFFER_SIZE-cnt);					
			ret=flashPageWrite(addr+pageIndex*PAGE_SIZE,protocolBuffer);
			pageIndex++;
			memcpy(&protocolBuffer,pData+(DATA_BUFFER_SIZE-cnt),size-(DATA_BUFFER_SIZE-cnt));
			cnt=size-(DATA_BUFFER_SIZE-cnt);
		}
...
//            offsetCheck=0;						
				...
				memcpy(&protocolBuffer[cnt],&crc,DATA_BUFFER_SIZE-cnt);
				flashPageWrite(addr+pageIndex*PAGE_SIZE,protocolBuffer);
...
U8 profileNameCheck(U8 id)
{
  ...
  U16 crc;
  U16 crc_cal;
  if(ramPr.aliveProfile&(1<<id)){    // If the profile is active and the profile name is settle success
    addr=ramPr.profile[id-1].nameAdd;
    nameSize=ramPr.profile[id-1].nameSize;
    if((addr!=0)||(nameSize!=0)){
       ret=0;                                                                    // retrun value false till the crc match
       crc_cal=crc16((U8*)ramPr.profile[id-1].nameAdd,ramPr.profile[id-1].nameSize);  // Calculate the crc 
...
void checkAllProfileName(void)
{
  ...
  U32 nameSize;
	for(i=0;i<8;i++){
		if((ramPr.aliveProfile>>i)&0x01){                 // The profile is alive
      if(!profileNameCheck(i)){                         // Profile name check fail
       nameSize=ramPr.profile[i-1].nameSize+CRC_SIZE;
       if(ramPr.profile[i-1].nameAdd&EXTER_ADDR_FLAG){     // Extern flash
				 macroTable.recycleSizes+=(nameSize/SECTOR_SIZE+1)*SECTOR_SIZE;
			 }else{
				 macroTable.recycleSizes+=(nameSize/PAGE_SIZE+1)*PAGE_SIZE;
			 }	
        ramPr.profile[i-1]=dft_generic2.profile[i-1]; // Profile become defautl setting 
        ramPr.aliveProfile&=~(1<<i);                  // Disable the profile

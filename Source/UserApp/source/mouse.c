...
U8  Hid_Ep1_Report_In[MAX_EP_SIZE];
U8  Hid_Ep2_Report_In[MAX_EP_SIZE];
U8  Hid_Ep3_Report_In[MAX_EP_SIZE];




LIFTOFF_SENSOR_PARM IRSensor;
CPI_PARM dpiCurrent;
  ...
  if (Hid_Ep2_Report_In[0] == REPORT_ID_RAZER) {          // Razer key event, we only has 3 key total, assign them to idx 1 2 3 to EP buffer
    ...
    if (buttonStatus[BTN_DPIUP_IDX].active) {
      Hid_Ep2_Report_In[i++] = DPI_UP_KEY;
		...
		if(ram.ambidextrous==RIGHT_HAND){                   // Thumb button for right hand
...
Arguments:
  ...
  Profile 1/2/3: ramPr.profile[ramPr.PClass].irSensor
...
--*/
{
  if(function == 0x01) {                                    // copy ram function to ram that will be require save
	  ramPr.profile[ramPr.ProNO].irSensor = IRSensor;
		ramPr.profile[ramPr.ProNO].dpiCur = dpiCurrent;
  } else if(function == 0x00) {                              // copy the saved function from ram to active
	 	IRSensor = ramPr.profile[ramPr.ProNO].irSensor;
	  dpiCurrent = ramPr.profile[ramPr.ProNO].dpiCur;
  }
}
...
--*/
{	
  if (optical >= IRSensor.pollingRate) {
...
//  spiWriteByte(REG_RESOLUTION_H, (y/50)>> 8); 
//	spiWriteByte(REG_RESOLUTION_L, (U8)(y/50));
//  
//  spiWriteByte(REG_CONFIG5_H,(x/50)>> 8);
//	spiWriteByte(REG_CONFIG5_L,(U8)(x/50));
SPI_W_2BYTE(0x1b, (U8)x);
//]  
...
//        spiWriteByte(REG_LIFT_CONFIG,0x02);               // Enable Present Cal and set lift to Universal 3mm		                                     
//        CAL_LiftConfig = 0x02;
//        spiWriteByte(REG_PIXEL_THRESHOLD,0x0A);           // Set Pixel Threshold to default Pixel Threshold
//        spiWriteByte(REG_LIFTCUTOFF_CAL1, 0x80);          // start the calibration procedure
//        calStatus = CAL_S_START;
//      break;
//      case CAL_S_START:
//				break;
//      case CAL_S_END:
//        CALSTAT = spiReadByte(REG_LIFTCUTOFF_CAL1) & 0x07;
//				if (( CALSTAT == 0x02) || (CALSTAT == 0x03)) {
//					spiWriteByte(REG_LIFTCUTOFF_CAL1, 0);              // stop the calibration procedure
//					delayMilliseconds(2);                              // wait 1msec
//					CAL_LiftCal2 = (spiReadByte(REG_LIFTCUTOFF_CAL2) & 0x7F);   // Reg 0x65
//					CAL_LiftCal3 = spiReadByte(REG_LIFTCUTOFF_CAL3);   // Reg 0x41
//					CAL_LiftCal4 = spiReadByte(REG_LIFTCUTOFF_CAL4);   // Reg 0x77
//					spiWriteByte(REG_CONFIGURATION_II,0x04);
...
//      case CAL_S_ABORT:
//        spiWriteByte(REG_LIFTCUTOFF_CAL1, 0);              // stop the calibration procedure
//        spiWriteByte(REG_LIFT_CONFIG,0x02);               // To disable Preset Calibration, and set lift to Universal 2mm
//        spiWriteByte(REG_PIXEL_THRESHOLD,0x0A);
//        spiWriteByte(REG_MIN_SQUAL_RUN,0x10);             // reset manual calibration
//        spiWriteByte(REG_CONFIGURATION_II,0x04);
...
//{
//  spiWriteByte(REG_LIFT_CONFIG,config);                   // write to register 0x63
//  spiWriteByte(REG_PIXEL_THRESHOLD,cal2);                 // write register 0x65 data to register 0x2C
//  spiWriteByte(REG_MIN_SQUAL_RUN,cal3);                   // write register 0x41 data to register 0x2B
//  spiWriteByte(REG_SQUAL_THRESHOLD,cal4);                 // write to register 0x76
//  spiWriteByte(REG_CONFIGURATION_II,0x04);
//}

...
{ 
  U8 i;
	currentButtons = 0;
  memcpy((U8*)&btnAssign,(U8*)&ramPr.profile[ramPr.ProNO].btnAssign,sizeof(btnAssign));
...
//    breakDeb = 0;
//    makeMsk = ram.makeMsk;
//    breakMsk = ram.breakMsk;
//  } else {                   //   lift off case 
    makeDeb = ram.makeDebounce;
    breakDeb =ram.breakDebounce;
    makeMsk = ram.makeMsk-makeDeb;
    breakMsk = ram.breakMsk-breakDeb;
				...
				switch(btnAssign[btnIdx].d1[modeData]){
					case BTN_STAGE_UP:
						if(eventType == BTN_MAKE){
							 ...
							 setResolution(dpiCurrent.x,dpiCurrent.y);
							 ramPr.profile[ramPr.ProNO].irSensor.dpiStage = IRSensor.dpiStage;
							 ramPr.profile[ramPr.ProNO].dpiCur.x = dpiCurrent.x;
							 ramPr.profile[ramPr.ProNO].dpiCur.y = dpiCurrent.y;
							 requestVariableUpdate(USER_CHANGED,ramPr.ProNO);
						}
						break;
					case BTN_STAGE_DN:
							if(eventType == BTN_MAKE){
...
//				}
		...
		case BTN_MACRO_II:                                // Macro typ II ,Display macro till button is released
       	...
       	if(eventType == BTN_MAKE){                        // Make even
			...
			case BTN_POWER:
				if(eventType == BTN_MAKE){
					Hid_Ep2_Report_In[0]=0x03;         // Media report id :0x02
			...
			case BTN_MEDIA:
				if(eventType == BTN_MAKE){
					Hid_Ep2_Report_In[0]=0x02;         // Media report id :0x02
...
//						turbo.btn.buttonValue |= btnAssign[btnIdx].d1[modeData];
	 ...
	 if(btnIdx<=4){                                          // Left ,Right,Middle,Back ,Forwd button
		 	if (eventType == BTN_MAKE) {
				currentButtons |= (1 << (dft_generic2.profile[ramPr.ProNO].btnAssign[btnIdx].d1[modeData] - 1));
			} else {
				currentButtons &= (~(1 << (dft_generic2.profile[ramPr.ProNO].btnAssign[btnIdx].d1[modeData] - 1)));
			}
			mouseEvent |= EVENT_BUTTON;                         // Set new Button information to report
...
void GetBtnAssgin(U8 profile ,U8 mode,U8 offset, U8* pData,U8 size)
{
	...
	for(i=offset;(i<offset+size)&&(i<NUMBER_OF_BUTTONS);i++){
		*(pData++)=ramPr.profile[profile-1].btnAssign[i].id;
		//*(pData++)=ramPr.profile[profile].btnAssign[i].mode;
		*(pData++)=ramPr.profile[profile-1].btnAssign[i].type[mode];
		*(pData++)=ramPr.profile[profile-1].btnAssign[i].size[mode];
		*(pData++)=ramPr.profile[profile-1].btnAssign[i].d1[mode];
		*(pData++)=ramPr.profile[profile-1].btnAssign[i].d2[mode];
		*(pData++)=ramPr.profile[profile-1].btnAssign[i].d3[mode];
		*(pData++)=ramPr.profile[profile-1].btnAssign[i].d4[mode];
		*(pData++)=ramPr.profile[profile-1].btnAssign[i].d5[mode];
	}
}

...
void	HID_SetReportEvent(void)
{
	...
	//@20170817 shufu_le
	else if( (sUSB_EumeData.bUSB_bmRequestType == HID_REQUEST_SET) && (sUSB_EumeData.bUSB_wIndexL == 1) )	//Interface 1
	{
		if (sUSB_EumeData.bUSB_wValueH == HID_REPORT_OUTPUT)
		{ //TYPE = OUTPUT
			HID_SetReportOutputEvent();
...
//  uint8_t i =0 ;
    if( (sUSB_EumeData.bUSB_wValueH == 0x03) && (sUSB_EumeData.bUSB_wIndexL == 0x00) && 
      (sUSB_EumeData.bUSB_wValueL == 0x00) )
    {
      wUSB_TableLength = 64;
      pUSB_TableIndex  = protocolTransmit;
      USB_TableTransmit();
    } else {
